...
# time
t1 <- strptime("01/01/2017 00:00:00", "%m/%d/%Y %H:%M:%S")
t2 <- t1+0.5*60*60 # 30 minutes after t1
time <- c(t1,t2)
# spatial coordinates
x <- c(0,2)
y <- c(1,4)
...
crs_NL = CRS("+init=epsg:28992")
# create class STIDF
stidf1 = STIDF(SpatialPoints(cbind(x,y),crs_NL), time1, data.frame(co2 = rnorm(n),O2=rnorm(n)))
...
STP_track1<-STP_Track(my_track1,v1)
STP_plot(STP_track1,time_interval = 0.3,zfactor = 2)

axes_STP_plot(time,z_factor = 2,n_ticks_xy = 2,n_ticks_z = 4)
